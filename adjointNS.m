function out = adjointNS(in)
% ADJOINTNS 
% Computes adjoint operator for non-square (and hence adjointNS) linear 
% chebops with no specified boundary conditions.
% Written by Gokul Hariharan, harih020@umn.edu.

Lin = linearize(in,0*in.init);
pref = chebfunpref();
[~,hand] = adjointFormal(Lin,pref);
out = chebop(in.domain);
out.op = hand;
end
