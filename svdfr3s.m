function varargout = svdfr3s(A,varargin)
%% SVDFR3i
% Computes the inverse problem for singular values corresponding to
% frequency responses, for linear block matrix operator of the form,
%
...
%
% Usage is very similar to chebop/eigs.m, most of these comments on usage are
% taken from chebop/eigs.m.
%
% D = SVDFR3(A, B, C) returns a vector of 6 eigenvalues of the system
% represented by A, B and C. SVDFR3 will attempt to return the
% singular values corresponding to the least
% oscillatory singular functions, similar to chebop/eigs.m.
%
% [V, D] = SVDFR3(A,B,C) returns a diagonal 6x6 matrix D of A's least oscillatory
% singular values, and a quasimatrix V of the corresponding singular functions.
%
% SVDFR3(A, B, C, K) for an integer K > 0 finds the K smoothest
% singular values.
%
% SVDFR3(A, B, C, K, SIGMA) finds K singular values. If SIGMA is a
...
%
% SVDFR3(..., PREFS) accepts a CHEBOPPREF to control the behavior of the
...
if (sigflag)
    [V,lam] = eigs(M,L,numval,pref);
else
    [V,lam] = eigs(M,L,numval,sigma, pref);
end
